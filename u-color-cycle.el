...
;;  Time-stamp: "25. März 2007, 19:09:00 (ulf)"
;;  Created:    Summer 2000
;;  Keywords:   Games, Fun, Eye-Candy
;;  Version:    $Id$

(defconst u-color-cycle-version 2.1
...
(defun u-color-cycle-create-faces (num)
  ...
  (let* ((i 0)
    ...
    (while (< i num)
      ...
      (setq c-face-name (intern (format "u-color-cycle-face-%d" i)))
      (setq c-face (make-face c-face-name))
...
(defun u-color-cycle-create-faces-gray (num)
  ...
  (let* ((i 0)
    ...
    (while (< i num)
      ...
      (setq c-face-name (format "u-color-cycle-face-%d" i))
      (setq c-face-name (intern (format "u-color-cycle-face-%d" i)))
      (setq c-face (make-face c-face-name))
      (setq u-color-cycle-face-vec (vconcat u-color-cycle-face-vec
					    (list c-face)))
...
DURATION make the fading slower. Must be larger than 0; it defaults to 10."
  ...
  (let (face-name (face-list (list)))
	...
	(setq new-face (make-face (intern face-name)))
